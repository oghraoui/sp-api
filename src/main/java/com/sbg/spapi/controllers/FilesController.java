...
public class FilesController {
    ...
    @PostMapping("/upload")
    public ResponseEntity<SPDocument> uploadFile(@RequestParam("type") String type,
                                     @RequestParam("module") String module,
                                     @RequestParam(value = "project", required = false) String project,
                                     @RequestParam("file") MultipartFile file) throws Exception {
        return ResponseEntity.ok(graphService.uploadFile(type, module, Objects.isNull(project) ? "" : project, file.getOriginalFilename(), file.getInputStream()));
    }
